...
const O2_30 = 0.30; // 30% vs Earth's 21%
...
const shouldCooperate_kin = LAWS.gameTheory.cooperation.shouldCooperateKin(relatedness, benefit, cost);
const shouldCooperate_nonkin = LAWS.gameTheory.cooperation.shouldCooperateNonKin(0.8, 0.9, benefit, cost, 0.1);

console.log(`Benefit: ${benefit}, Cost: ${cost}`);
console.log(`Kin cooperation (r=0.125): ${shouldCooperate_kin ? 'YES' : 'NO'} (Hamilton: r×B=${relatedness * benefit} ${relatedness * benefit > cost ? '>' : '<'} C=${cost})`);
console.log(`Non-kin cooperation: ${shouldCooperate_nonkin ? 'YES' : 'NO'} (reciprocal altruism)\n`);
...
const yield_kgha = LAWS.agriculture?.yields?.potentialYield_kgPerHa?.(GDD, rainfall, NPK, sunlight) || 0;
...
console.log(`Result: ${areaNeeded_ha < 50 ? 'VIABLE' : 'NOT VIABLE'} (${areaNeeded_ha < 50 ? 'can sustain population' : 'too much land needed'})\n`);
