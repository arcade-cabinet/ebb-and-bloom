...
async function runFullSimulation() {
  ...
  for (let cycle = 1; cycle <= cycles; cycle++) {
    ...
    if (cycle % 10 === 0) {
      ...
      console.log(`Cycle ${cycle.toString().padStart(3)}: Pop=${totalPop.toString().padStart(5)} | ${govType.type.padEnd(10)} | EQ=${EQ.toFixed(2)} | Tools=${canUseTool ? 'YES' : 'NO '} | Species=${species.filter(s => s.population > 0).length}/${species.length}`);
    }
  }
  ...
  console.log(`  Growth: ${((last.totalPopulation / first.totalPopulation - 1) * 100).toFixed(1)}%`);
