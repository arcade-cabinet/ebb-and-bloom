...
export class YukaGuidedGeneration {
   ...
   */
  private static generatePlanet(props: {
    composition: any;
    temperature: number;
    atmosphere: any;
  }) {
    ...
    console.log(`  Crust: ${Object.entries(crust).slice(0, 3).map(([e, f]) => `${e}:${((f as number) * 100).toFixed(0)}%`).join(', ')}`);
    console.log(`  Color: RGB(${visuals.baseColor.r.toFixed(2)}, ${visuals.baseColor.g.toFixed(2)}, ${visuals.baseColor.b.toFixed(2)})`);
   ...
   */
  private static generateCreature(props: {
    mass: number;
    locomotion: string;
    diet: string;
    biome: string;
    ...
    console.log(`  Mass: ${props.mass.toFixed(1)}kg → Body: ${visuals.proportions.bodyLength.toFixed(2)}m`);
   ...
   */
  private static generateTool(props: {
    material: string;
    purpose: string;
    techLevel: number;
  }, age: number) {
   ...
   */
  private static generateStructure(props: {
    materials: string[];
    construction: string;
    population: number;
  }, age: number) {
