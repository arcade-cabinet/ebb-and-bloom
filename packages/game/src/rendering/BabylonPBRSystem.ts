...
export class BabylonPBRSystem {
   ...
   */
  private static calculateEmissive(comp: Record<string, number>, temp_K: number): {
    color: Color3;
    intensity: number;
  } | null {
  ...
  static createAtmosphereParticles(
    ...
    if (CO2 > 0.5) {
      particles.color1 = new Color3(0.9, 0.6, 0.4); // Orange (CO₂)
      particles.color2 = new Color3(0.95, 0.7, 0.5);
    } else {
      particles.color1 = new Color3(0.5, 0.7, 1); // Blue (N₂/O₂)
      particles.color2 = new Color3(0.6, 0.8, 1);
    }
  ...
  static createProceduralNormals(
    ...
    const proc = new ProceduralTexture(name, 512, 
      `
