...
export class NPCSpawner {
  ...
  private spawnNPC(settlement: Settlement, index: number, rng: EnhancedRNG): void {
    ...
    agent.maxSpeed = rng.uniform(3, 5); // FASTER walking speed
   ...
   */
  private createNPCMesh(role: NPCRole, rng: EnhancedRNG): THREE.Mesh {
   ...
   */
  private createPath(from: THREE.Vector3, to: THREE.Vector3): Path {
    const path = new Path();
