...
export class BiomeSystem {
  ...
  blendBiomeColors(x: number, z: number, altitude: number, blendRadius: number = 5): THREE.Color {
    // Sample 4 points around the position
    const samples = [
      this.getBiome(x, z, altitude),
      this.getBiome(x + blendRadius, z, altitude),
      this.getBiome(x, z + blendRadius, altitude),
      this.getBiome(x - blendRadius, z - blendRadius, altitude)
    ];
