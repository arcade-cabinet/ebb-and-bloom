...
export const Scattering = {
   ...
   */
  skyColor: (sunAngle_deg: number, atmosphericDensity: number): { r: number; g: number; b: number } => {
  ...
  mieScattering: (particleSize_um: number, wavelength_nm: number): number => {
    ...
    if (x < 0.1) return this.rayleighIntensity(wavelength_nm, 1); // Small → Rayleigh
...
export const Absorption = {
  ...
  perceivedColor: (absorbedWavelengths_nm: number[]): { r: number; g: number; b: number } => {
    ...
    for (const lambda of absorbedWavelengths_nm) {
      if (lambda < 500) b *= 0.3; // Absorb blue
      else if (lambda < 600) g *= 0.3; // Absorb green
   ...
   * I = I₀ × e^(-α × d)
   */
  transmission: (initialIntensity: number, absorptionCoeff: number, thickness_m: number): number => {
