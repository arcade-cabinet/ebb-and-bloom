...
export const PlateTectonics = {
  ...
  plateVelocity_cmPerYear: (
    ...
    const velocity = 5 * (earthViscosity / mantleViscosity) * (temperatureGradient_KperM / earthGradient);
...
export const Volcanism = {
   ...
   */
  VEI: (
    eruptionVolume_km3: number,
    eruptionDuration_hours: number
  ): number => {
    // VEI based on volume
...
export const RockCycle = {
  ...
  types: {
    ...
    sedimentary: {
      ...
      composition: { Si: 0.35, O: 0.48, Ca: 0.10, C: 0.05 },
      hardness: 3,
    },
    metamorphic: {
      formation: 'Heat and pressure transformation',
      composition: { Si: 0.30, O: 0.48, Al: 0.10, Fe: 0.06 },
      hardness: 7,
    },
  },
...
export const Erosion = {
   ...
   */
  totalErosion: (
    physical_mmPerYear: number,
    chemical_mmPerYear: number
  ): number => {
