...
export const GrowthPerformance = {
   ...
   */
  productionValueIndex: (
    SGR_percentPerDay: number,
    FCR: number,
    survivalRate: number
  ): number => {
    // Higher is better
...
export const SelectiveBreeding = {
  ...
  traitAfterGenerations: (
    ...
    const responsePerGen = SelectiveBreeding.responseToSelection(heritability, selectionDifferential);
  ...
  typicalHeritability: (trait: string): number => {
    const heritabilities: Record<string, number> = {
      'body_size': 0.40, // Moderately heritable
      'growth_rate': 0.30,
      'feed_efficiency': 0.25,
      'disease_resistance': 0.15,
      'fertility': 0.10, // Low heritability
      'behavior': 0.20,
      'coat_color': 0.90, // Highly heritable
    };
...
export const ProductionModels = {
  ...
  eggProduction: (
    ...
    const rate = (Math.pow(x, shape - 1) * Math.exp(-x / scale)) / 
