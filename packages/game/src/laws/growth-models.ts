...
export const LogisticGrowth = {
   ...
   * dW/dt = r × W × (1 - W/W_∞)
   */
  growthRate: (currentMass_kg: number, W_infinity_kg: number, r_perYear: number): number => {
...
export const SeasonalGrowth = {
   ...
   */
  growingDegreeDays: (dailyTemperatures_K: number[], thresholdTemp_K: number): number => {
   ...
   */
  growingSeasonLength: (yearlyTemperatures_K: number[], thresholdTemp_K: number): number => {
...
export const IndeterminateGrowth = {
   ...
   * rate(t) = K × (1 - L(t)/L_∞)
   */
  annualGrowthRate: (currentSize_m: number, asymptoteSize_m: number, K_perYear: number): number => {
