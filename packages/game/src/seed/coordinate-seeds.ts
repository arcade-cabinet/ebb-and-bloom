...
import { SpacetimeCoordinates } from '../simulation/UniverseSimulator';

// Word lists for readable seeds
const ADJECTIVES = [
  'red', 'blue', 'green', 'ancient', 'bright', 'dark', 'wild', 'calm',
  'fierce', 'gentle', 'bold', 'subtle', 'vivid', 'pale', 'deep', 'light',
  'young', 'old', 'fresh', 'eternal', 'frozen', 'burning', 'still', 'swift',
  'hidden', 'shining', 'silent', 'roaring', 'distant', 'near', 'strange', 'familiar',
];

const NOUNS = [
  'star', 'moon', 'sun', 'nebula', 'void', 'storm', 'ocean', 'mountain',
  'forest', 'desert', 'crystal', 'stone', 'wind', 'fire', 'ice', 'water',
  'comet', 'planet', 'galaxy', 'cosmos', 'aurora', 'eclipse', 'horizon', 'zenith',
  'dawn', 'dusk', 'tide', 'wave', 'peak', 'valley', 'river', 'cloud',
];

const VERBS = [
  'dance', 'sing', 'soar', 'flow', 'glow', 'shine', 'bloom', 'burn',
  'freeze', 'melt', 'rise', 'fall', 'spin', 'drift', 'pulse', 'flare',
  'whisper', 'echo', 'shimmer', 'blaze', 'surge', 'wane', 'gleam', 'fade',
  'spiral', 'orbit', 'merge', 'split', 'ascend', 'descend', 'transform', 'endure',
];
...
export function validateBidirectional(seed: string): boolean {
  ...
  return seed === regenerated || 
         seed.split('-').every(word => regenerated.includes(word));
}
