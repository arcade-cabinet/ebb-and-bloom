...
export class ModeTransitionSystem {
  ...
  private checkModeTransition(
    ...
    if (previousDistance > ZOOM_THRESHOLDS.enterGameMode && 
    ...
    if (previousDistance <= ZOOM_THRESHOLDS.exitGameMode &&
  ...
  private transitionToGameMode(coords: SpacetimeCoordinates): void {
    ...
    console.log(`[Mode] Zoomed into coordinates [${coords.x}, ${coords.y}, ${coords.z}] at t=${coords.t}`);
  ...
  zoomBackIntoSavedPlanet(seed: string): void {
    ...
    console.log(`[Mode] Time elapsed: ${(coords.t - this.targetCoordinates!.t) / (365.25 * 86400)} years`);
  }
