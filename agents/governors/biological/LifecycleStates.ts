...
export class LifecycleStateMachine extends StateMachine {
    constructor(agent: any) {
        ...
        // Add states (Yuka StateMachine API - using type assertion for proper typing)
        const juvenileState = new JuvenileState();
        const adultState = new AdultState();
        const elderState = new ElderState();

        (this as any).add('juvenile', juvenileState);
        (this as any).add('adult', adultState);
        (this as any).add('elder', elderState);

        // Set initial state based on age
        if (agent.age < maturityAge) {
            (this as any).changeTo('juvenile');
        } else if (agent.age < maxLifespan * 0.75) {
            (this as any).changeTo('adult');
        } else {
            (this as any).changeTo('elder');
        }
    }
    ...
    update(delta: number): void {
        const agent = (this as any).owner;

        if (!agent) return;

        // Age the agent (delta is in seconds, convert to years)
        agent.age = (agent.age || 0) + delta / (365.25 * 24 * 3600); // seconds → years
