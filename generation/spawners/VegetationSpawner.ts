...
// TreeInstance type - used for type safety in instance management
// Instances are stored in THREE.InstancedMesh, but this type represents the data structure
...
export class VegetationSpawner {
  ...
  spawnInChunk(
    ...
    // Use instance rng for global vegetation density variations
    const globalDensityVariation = this.rng.uniform(0.9, 1.1);
    ...
    // Calculate tree count based on density (with global variation)
    const baseTreeCount = 20; // Per chunk at full density
    const adjustedDensity = config.treeDensity * globalDensityVariation;
    const treeCount = Math.floor(baseTreeCount * adjustedDensity);
