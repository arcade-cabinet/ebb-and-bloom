...
async function demonstrateSynthesis() {
  ...
  console.log(
    `Surface temp: ${planet.surfaceTemp.toFixed(1)} K (${(planet.surfaceTemp - 273.15).toFixed(1)}°C)\n`
  );
  ...
  if (creatures.length > 0) {
    ...
    console.log(
      `    Metabolic rate: 70 × ${creature.mass.toFixed(1)}^0.75 = ${(70 * Math.pow(creature.mass, 0.75)).toFixed(1)} W`
    );
    console.log(
      `    Heart rate: 241 × ${creature.mass.toFixed(1)}^-0.25 = ${(241 * Math.pow(creature.mass, -0.25)).toFixed(1)} BPM`
    );
    console.log(
      `    Lifespan: 10.5 × ${creature.mass.toFixed(1)}^0.25 = ${(10.5 * Math.pow(creature.mass, 0.25)).toFixed(1)} years\n`
    );
  ...
  console.log(
    '  1. Crust composition → Fe content = ' + ((crustElements.Fe || 0) * 100).toFixed(1) + '%'
  );
  console.log(
    '  2. Reduction temp → ' +
      (LAWS.metallurgy?.smelting?.reductionTemperature?.('Fe') || 1811) +
      'K needed'
  );
  console.log(
    '  3. Fire capability → ' +
      (LAWS.combustion?.chemistry?.flameTemperature_K?.('charcoal', 21) || 1473) +
      'K achievable'
  );
