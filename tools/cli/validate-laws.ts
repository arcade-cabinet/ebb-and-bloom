...
async function runValidation() {
  ...
  console.log(
    `Match: ${data1a.universe.star.mass === data1b.universe.star.mass ? '✅ PASS' : '❌ FAIL'}`
  );

  console.log(`\nData 1A planet count: ${data1a.universe.planets.length}`);
  console.log(`\nData 1B planet count: ${data1b.universe.planets.length}`);
  console.log(
    `Match: ${data1a.universe.planets.length === data1b.universe.planets.length ? '✅ PASS' : '❌ FAIL'}`
  );
  ...
  if (data1a.universe.planets.length > 0) {
    ...
    console.log(
      `Match: ${data1a.universe.planets[0].mass === data1b.universe.planets[0].mass ? '✅ PASS' : '❌ FAIL'}`
    );
  }
  ...
  console.log(
    `Expected: ~2.5 | Result: ${Math.abs(meanPlanets - 2.5) < 0.5 ? '✅ PASS' : '❌ FAIL'}`
  );
  ...
  for (const mass of testMasses) {
    ...
    console.log(
      `${mass.toString().padStart(9)} | ${bmr.toFixed(2).padStart(19)} | ${match ? '✅' : '❌'}`
    );
  }
  ...
  for (const mass of testMasses) {
    ...
    console.log(
      `${mass.toString().padStart(9)} | ${hr.toFixed(1).padStart(16)} | ${mass === 1 ? 'Mouse' : mass === 10 ? 'Dog' : mass === 100 ? 'Human' : 'Whale'}`
    );
  }
  ...
  for (const mass of testMasses) {
    ...
    console.log(
      `${mass.toString().padStart(9)} | ${herbivore.toFixed(3).padStart(15)} | ${carnivore.toFixed(3).padStart(15)} | ${ratio.toFixed(2)}`
    );
  }
  ...
  species.forEach((sp, i) => {
    console.log(
      `  ${i + 1}. ${sp.scientificName}: ${sp.population} individuals ${sp.population === 0 ? '💀 EXTINCT' : ''}`
    );
  });
  ...
  crossGenSpecies.slice(0, 3).forEach((sp, i) => {
    ...
    console.log(
      `     Expected BMR: ${BiologicalLaws.allometry.basalMetabolicRate(sp.mass || 50).toFixed(1)} W`
    );

    const bmrMatch =
      Math.abs((sp.metabolism || 0) - BiologicalLaws.allometry.basalMetabolicRate(sp.mass || 50)) <
      5;
