...
class ProductionAssetGenerator {
  ...
  private manifestPath = './manifests/production-assets.json';
  ...
  private async loadAmbientCGCatalog(): Promise<void> {
    try {
      ...
      const manifestPath = './public/textures/manifest.json';
...
Brand Integration: Subtle earth tones with hints of Ebb & Bloom color palette (deep indigo, emerald green, trait gold accents where appropriate).`;
    ...
    try {
      ...
      const filePath = `./public/creatures/${fileName}`;
      
      // Ensure directory exists
      if (!existsSync('./public/creatures')) {
        await mkdirSync('./public/creatures', { recursive: true });
      }
