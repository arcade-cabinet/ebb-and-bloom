...
export const RadiativeBalance = {
   ...
   */
  equilibriumTemperature: (solarFlux_Wm2: number, albedo: number = 0.3): number => {
...
export const GreenhouseEffect = {
   ...
   */
  surfaceTemperature: (equilibriumTemp_K: number, opticalDepth: number): number => {
   ...
   */
  opticalDepth: (CO2_ppm: number, CH4_ppm: number, H2O_percent: number): number => {
   ...
   */
  deltaTemperature: (initialCO2_ppm: number, finalCO2_ppm: number): number => {
...
export const MilankovitchCycles = {
   ...
   */
  insolationVariation: (time_years: number, baseInsolation_Wm2: number): number => {
    // Eccentricity component
    const ecc =
      0.05 * Math.sin((2 * Math.PI * time_years) / MilankovitchCycles.eccentricityCycle_years);

    // Obliquity component
    const obl =
      0.03 * Math.sin((2 * Math.PI * time_years) / MilankovitchCycles.obliquityCycle_years);

    // Precession component
    const prec =
      0.02 * Math.sin((2 * Math.PI * time_years) / MilankovitchCycles.precessionCycle_years);
...
export const Albedo = {
   ...
   * Runaway process!
   */
  iceAlbedoFeedback: (temperature_K: number, currentIceCoverage: number): number => {
...
export const AtmosphericCirculation = {
   ...
   */
  numberOfCells: (rotationPeriod_hours: number, planetRadius_m: number): number => {
