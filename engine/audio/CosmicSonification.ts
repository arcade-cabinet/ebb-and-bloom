...
export class CosmicSonification {
   ...
   */
  sonicateStellarFusion(stellarMass_Msun: number, stellarAge_Gyr: number): void {
    ...
    console.log(
      `[Sonification] Stellar fusion: ${stellarMass_Msun.toFixed(3)}M☉ → ${baseFreq.toFixed(1)}Hz drone`
    );
   ...
   */
  sonificationSolarWind(particleDensity_perM3: number, velocity_ms: number): void {
    ...
    console.log(
      `[Sonification] Solar wind: ${velocity_ms}km/s → Whoosh at ${filter.frequency.value.toFixed(0)}Hz`
    );
   ...
   */
  sonifyGravitationalWave(initialFreq_Hz: number, finalFreq_Hz: number, duration_s: number): void {
  ...
  atmosphericSoundBegins(
    ...
    console.log(
      `[Audio] Speed of sound: ${this.calculateSoundSpeed(composition, temperature_K).toFixed(0)} m/s`
    );
  ...
  private createWhiteNoise(duration_s: number): AudioBufferSourceNode {
    const buffer = this.audioContext.createBuffer(
      1,
      this.audioContext.sampleRate * duration_s,
