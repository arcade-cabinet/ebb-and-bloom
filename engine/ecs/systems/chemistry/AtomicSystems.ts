...
import { ChemistryQueries } from '../../queries/ChemistryQueries';
...
export const gravitySystem = (world: World<AtomEntity>, delta: number) => {
  const atoms = world.with('position', 'element');
...
export const bondingSystem = (world: World<AtomEntity>) => {
  const atoms = world.with('element', 'position', 'id');
  const atomArray = Array.from(atoms);
...
// Wrapper class to match LawOrchestrator System interface
export class AtomicChemistrySystem {
  update(world: World<any>, delta: number): void {
    // Cast to AtomEntity world for atomic systems
    const atomicWorld = world as World<AtomEntity>;
    
    gravitySystem(atomicWorld, delta);
    bondingSystem(atomicWorld);
    bondPruningSystem(atomicWorld);
  }
}
