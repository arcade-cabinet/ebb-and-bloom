...
export class ElementalRenderer {
  ...
  calculateVisualProperties(
    ...
    // 6. EMISSIVE if hot
    const emissiveColor = temperature_K > 800 ? this.blackbodyColor(temperature_K) : undefined;
   ...
   */
  private colorFromComposition(composition: Record<string, number>, temperature_K: number): Color3 {
   ...
   */
  metallicFromComposition(composition: Record<string, number>): number {
...
export class PlanetSurfaceRenderer extends ElementalRenderer {
  ...
  createPlanetMaterial(
    ...
    const props = this.calculateVisualProperties(crustComposition, temperature_K, 'amorphous');
...
export class ToolRenderer extends ElementalRenderer {
  ...
  createToolMaterial(materialComposition: Record<string, number>, weathering: number, scene: any) {
    const structure =
      this.metallicFromComposition(materialComposition) > 0.5
        ? 'crystalline' // Metals
